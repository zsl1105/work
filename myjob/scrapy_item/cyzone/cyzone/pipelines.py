...
class CyzonePipeline(object):
    ...
    def do_insert(self, cursor, item):
        ...
        insert_sql = 'insert ignore into startup(name,area,address,web,start_time, state, introduction, company) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)'
        name_list = []
        for row in rows:
            name_list.append(row['name'])

        if item['name'] not in name_list:
            cursor.execute(insert_sql, (item['name'], item['area'], item['address'], item['web'],
                                        item['datetime'], item['stage'], item['briefs'], item['company']))
            print(item)
            print('***********************')
        else:
